...
class ExtractObjects:
    def extract(self, studentID, entryOrExit = True):
        ...
        #         #read all objects of the person
        #         #cvzone.cornerRect(image, (x1, y1, w, h))
        #         #cvzone.putTextRect(image, f'{Objects.getClassname(Objects,int(cls))} {conf}%', (max(0,x1),max(35, y1-20)), scale=1)

        #         #confidence
        #         if conf <= 60:
        #             print(f'Confidence level of {Objects.getClassname(Objects,int(cls))} below set level of 60%')
        #             continue

        #         #check for specific things like laptop, phone
        #         if(cls!=0 and cls!=63 and cls!=66 and cls!=67):
        #             continue

        #         os.makedirs(directory, exist_ok=True)  # Create the directory if it doesn't exist
        #         filename = f'{Objects.getClassname(Objects,int(cls))}.png'
        #         path = os.path.join(directory, filename)
        ...
        #             filename = f'{Objects.getClassname(Objects,int(cls))}-{count}.png'
        #             path = os.path.join(directory, filename)
        ...
        #             objCounts+=1
        #             if objCounts == 1:
        #                 print("SAVING = ", util.saveToDb(util() , studentID, filename, entryOrExit))
        #             else:
        #                 print("UPDATING = ", util.updateDb(util(), studentID, objCounts, filename, entryOrExit))


        #         #cropp image and save it
        #         cropped_image = image[y1:y2, x1:x2]
        #         cv2.imwrite(path, cropped_image)

        #     if(objCounts==0):
        #         print("COULD NOT FIND ANY ELECTRONICS ON YOU")
        #         print("SAVING = ",  util.saveToDb(util(), studentID, None, entryOrExit))